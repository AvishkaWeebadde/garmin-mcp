...
- **Current phase**: Phase 1 essentially complete — Java server over stdio, wired into Claude Desktop, **real `.FIT` provider working against a live Strava export**. Phase 2 (Go) is next.
- **Last session**: 2026-08-21 — built + verified `FitFileActivityProvider` (6/6 synthetic tests + real 249-activity export), ADR-0012, findings F-010–F-012, distances-raw decision. Earlier same day: pushed to `github.com/AvishkaWeebadde/garmin-mcp`; wired into Claude Desktop via WSL (see wiring notes below).
- **Prev session**: 2026-08-05 — contract v1 frozen; Java built/verified on JDK 21 via `./mvnw`. 7/7 happy paths conform. ADRs 0001–0011.
- **Next**: Phase 2 (Go, streamable HTTP + stateless), same contract, same providers. Optional Phase-1 cleanups: real-multisport check, GPX adapter (F-012), the two open findings below.
- **Real export lives OUTSIDE the repo** at `F:\strave export\extracted\activities` (188 `.fit.gz`, personal data — never commit). Run: `python3 scripts/mcp_probe.py <jdk21>/java -Dfitmcp.provider=fit "-Dfitmcp.fit.directory=<dir>" -jar <jar>` from inside WSL.
...
- **Cold-start**: addressed 2026-08-21. Config-only fixes failed (`TieredStopAtLevel=1` backfires — indexing wants the JIT; `spring.main.lazy-initialization=true` breaks the MCP server, INIT FAILED). Real fix is provider code, now done: **(a) parallel + selective decode** — low-level `Decode` with `session`/`file_id` listeners only, skipping per-second `record` messages, across a parallel stream (index ~6s→~3.3s for 187 files); **(b) async indexing** — constructor returns immediately, `CompletableFuture.supplyAsync` decodes on the common pool, first query blocks via `activities()`/`index.join()`. So Claude Desktop connects fast and tools appear at once; only the first query waits. Side effect: lenient low-level decode recovers the truncated file → **188** indexed, not 187 (F-011 updated). Log path fixed via launch flag `-Dlogging.file.name=/mnt/f/fitmcp/fitmcp.log` (relative `./fitmcp.log` failed under Claude's launch cwd).
- **Open**:
